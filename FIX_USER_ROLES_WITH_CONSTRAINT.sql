-- ============================================================================
-- FIX USER ROLES - ADDRESSING CONSTRAINT ISSUE
-- ============================================================================
-- This script fixes user roles while handling the role check constraint

-- Step 1: First, let's see what the current constraint is
...
-- Step 2: Let's see what roles exist and what the constraint expects
SELECT 'Available roles:' as info;
SELECT id, name FROM public.roles ORDER BY name;

-- Step 3: Let's see current users and their roles
...
-- Step 4: Drop the problematic constraint temporarily
ALTER TABLE public.users DROP CONSTRAINT IF EXISTS users_role_check;

-- Step 5: Update specific users based on their email addresses
...
-- Step 6: Update all users with 'member' role to 'resident' role
UPDATE public.users 
SET role_id = (SELECT id FROM public.roles WHERE name = 'resident')
WHERE role_id = (SELECT id FROM public.roles WHERE name = 'member');

-- Step 7: Update any users without a role to 'resident' role
UPDATE public.users 
SET role_id = (SELECT id FROM public.roles WHERE name = 'resident')
WHERE role_id IS NULL;

-- Step 8: Recreate the constraint with proper role values
ALTER TABLE public.users 
ADD CONSTRAINT users_role_check 
CHECK (role_id IN (SELECT id FROM public.roles));

-- Step 9: Show the updated users and roles
...
-- Step 10: Verify the role distribution
...
-- Step 11: Show any users that still don't have proper roles
