...
export async function POST(req: Request) {
  try {
    ...
    // Compute PET-only total for the collection (use fixed R1.50/kg)
    ...
    const petRate = 1.5 // R1.50 per kg
    ...
    const { error: insErr } = await supabase
      ...
      .insert({
        ...
        description: `PET contribution from collection ${collectionId} @ R${petRate.toFixed(2)}/kg`
      })
